...
Ext.define('Contact.controller.Contacts', {
    ...
    onSaveContactBtnTap: function(button, e, eOpts) {
        ...
        var errors = [];
        var reqFields = form.query('field[required=true]');
        var i, ln = reqFields.length, field;
        for (i = 0; i < ln; i++) {
            field = reqFields[i];
            if (!field.getValue()) {
                errors.push(field.getLabel() + ' deve ser preenchido.');
            }
        }
        ...
        if (errors.length) {
            ...
            Ext.Msg.alert('Sucesso!', 'Contato Salvo com Sucesso! :)');
        }
